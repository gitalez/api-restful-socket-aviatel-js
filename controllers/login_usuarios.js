...
async function loginUsuarioGoogle(req, res) {
    ...
    Usuario.findOne({ email: googleUser.email }, (err, usuarioDB) => {
        ...
        if (usuarioDB) {
            ...
            if (usuarioDB.google === false) {
                ...
                return resp.status(400).json({ // internal server error
                    ok: false,
                    mensaje: 'debe autenticarse por su autentificacion de email y contraseña'
                });
            } else {
                // existe en usuarioBD autenticado por google 
                // generamos el token 
                var token = jwt.sign({ usuario: usuarioDB }, process.env.SEED, { expiresIn: process.env.CADUCIDAD_TOKEN });
